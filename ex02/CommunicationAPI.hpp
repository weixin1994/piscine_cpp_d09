...
class CommunicationAPI
{
    public:
        ...
        // Create a CommunicationAPI's instance from valid input and output stream or
        ...
        // Register the given user to this API. It will fail if the name is invalid
        // or if there is already too many users.
        void addUser(std::string const &userName);

        // Receive a message. The given user should be valid and the mission
        // started as well.
        void receiveMessage(std::string const &userName, std::string &message) const;

        // Send a message. The given user should be valid and the mission started
        // as well.
